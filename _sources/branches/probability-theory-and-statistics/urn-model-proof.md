...
Each outcome of the [Laplace experiment][bookofproofs$973] described in the urn model consists of `\(n\)` balls drawn from `\(N\)` balls, while the order of balls drawn does not play any role. Therefore, the number of possible outcomes is given by the [binomial coefficient][bookofproofs$993].
`\[|\Omega|=\binom Nn.\]`
