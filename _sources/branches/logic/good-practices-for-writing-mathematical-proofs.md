...
1. You have now two possibilities to continue the proof: 
   * Possibility 1: Direct Proof:
##1. Assert the hypothesis, i.e. take `$y$` and assume that `$P(y)$` is true.
##1. List implications, showing that then `$Q(y)$` must also be true.
##1. State the conclusion, e.g. "Therefore, it follows that `$P\Rightarrow Q.$`"
   * Possibility 2: Proof by contradiction: 
##1. Assert the hypothesis, i.e. take `$y$` and assume that both: `$P(y)$` is true and `$Q(y)$` is false.
##1. List implications, leading to that contradiction.
##1. State the conclusion, e.g. [This is a "contradiction][bookofproofs$744] to the hypothesis. Thus, it follows that `$P\Rightarrow Q.$`"
